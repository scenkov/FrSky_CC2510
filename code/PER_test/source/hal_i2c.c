...
#ifndef OCM_CLK_PORT
  #define OCM_CLK_PORT  2
#endif

#ifndef OCM_DATA_PORT
  #define OCM_DATA_PORT   2
#endif
...
// EEPROM WRITE byte
uint8_t hali2cWrite_byte( uint8 devaddr, uint16 regaddr, uint8 data )
{
  //uint8  ch;
 // uint16 i;
 //uint8_t d = 0;
...
// EEPROM WRITE page
uint8_t hali2c_write_page(uint8_t devaddr, uint16_t regaddr, uint8_t *data, uint8_t size)
{
  hali2cSendDeviceAddress((devaddr<<1) | OCM_WRITE);
  hali2cSendByte(regaddr >> 8);
...
void hali2cformateeprom(void){
  ...
  for (uint8 j = 0; j <= 32; j++) {
    buffer[j] = 0xff;
      ...
      for (uint16 i = 0; i <= SIZE_EEPROM_24C64 / 32; i++) {
        hali2c_write_page(ADDR_EEPROM_24C64, i*32, buffer, 32);
        if ( i%30  == 0 )
          P1_0 ^= 1;
        hali2c_acknowledge_write_polling(ADDR_EEPROM_24C64);
   //     delay_5ms();
      }
    hali2cWrite_byte(ADDR_EEPROM_24C64, 0x1ffe, EEPROM_MAGIC >> 8);
    hali2c_acknowledge_write_polling(ADDR_EEPROM_24C64);
   // delay_5ms();
    hali2cWrite_byte(ADDR_EEPROM_24C64, 0x1fff, EEPROM_MAGIC & 0xff);
    hali2c_acknowledge_write_polling(ADDR_EEPROM_24C64);
  //  delay_5ms();
}
